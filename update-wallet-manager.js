...
<body>
    ...
    async function checkStatus() {
        if (typeof window.ethereum === 'undefined') {
            statusDisplay.innerHTML = '<span style="color: #ff4444">❌ No Web3 wallet detected</span>';
            return;
        }
        ...
        try {
            ...
            if (accounts.length > 0) {
                ...
                // Update status
                statusDisplay.innerHTML = '<span style="color: #4CAF50">✅ Connected to MetaMask</span>';
                ...
                statusDisplay.innerHTML = '<span style="color: #ff9800">⚠️ Not connected to any wallet</span>';
                accountDetails.style.display = 'none';
                currentAccount = null;
            }
            
        } catch (error) {
            statusDisplay.innerHTML = \`<span style="color: #ff4444">❌ Error: \${error.message}</span>\`;
            accountDetails.style.display = 'none';
        }
    }
    ...
    async function connectMetaMask() {
        ...
        try {
            ...
            if (accounts.length > 0) {
                showNotification(\`✅ Connected: \${accounts[0].substring(0,6)}...\${accounts[0].substring(38)}\`);
                setTimeout(() => {
                    checkStatus();
                }, 1000);
            }
    ...
    async function disconnectWallet() {
        ...
        try {
            ...
            showNotification('✅ Disconnected. You can now connect a different wallet.');
            setTimeout(() => {
                checkStatus();
            }, 1500);
            ...
            showNotification('✅ Local connection cleared');
            currentAccount = null;
            checkStatus();
        }
    }
...
console.log('✅ Wallet manager updated with clean interface');
