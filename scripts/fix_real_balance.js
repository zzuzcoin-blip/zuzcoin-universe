console.log("🔄 Обновляем интерфейс для отображения реального баланса ZUZ...");

// Обновляем основной файл index.html
    ...
    <script>
        ...
        async function loadBalances() {
            try {
                ...
                // Get ZUZ balance
                zuzContract = new ethers.Contract(ZUZ_TOKEN_ADDRESS, ZUZ_TOKEN_ABI, provider.getSigner());
                const balance = await zuzContract.balanceOf(userAddress);
                const decimals = await zuzContract.decimals();
                zuzBalance = parseFloat(ethers.utils.formatUnits(balance, decimals));
                
                document.getElementById('zuzBalance').textContent = 
                    `${zuzBalance.toLocaleString('en-US', {maximumFractionDigits: 0})} ZUZ`;
...
console.log("📊 Теперь интерфейс будет показывать реальный баланс ZUZ из MetaMask");
